...
contract Dispute{
    ...
    function check_max(uint _id) public{
        uint uid = current_disputes[_id]._id;
        if(current_disputes[_id].weight_for*2 > total_weight)
        {
            uint amount = all_agreement[uid].value;
            uint sum = 0;
            for(uint i=0;i<current_disputes[_id].list_for.length;i++)
            {
                uint weight = address_to_weight[current_disputes[_id].list_for[i]];
                weight += 25;
                total_weight += 25;
                current_disputes[_id].list_for[i].transfer(amount*weight/(20*total_weight));
                sum += amount*weight/(20*total_weight);
            }
            all_agreement[uid].client.transfer(amount-sum);
        ...
        if(current_disputes[_id].weight_against*2 > total_weight)
        {
            uint amount = all_agreement[uid].value;
            uint sum = 0;
            for(uint i=0;i<current_disputes[_id].list_against.length;i++)
            {
                uint weight = address_to_weight[current_disputes[_id].list_for[i]];
                weight += 25;
                total_weight += 25;
            ...
            all_agreement[uid].service_provider.transfer(amount-sum);
            all_agreement[uid].status = false;
            current_disputes[_id].open = false;
        }
    }
    
    function arbitration(uint _id,bool _for) public{
